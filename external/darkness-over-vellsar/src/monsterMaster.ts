...
export module MonsterMaster {
    export function getItemMaster():Monster[] {
       return initItemlist();
    }
    function initItemlist() :Monster[]{
        const item: Monster[] = [
    ...
    export function getMonsterList(type:string,place:string,limit:number) :Monster[]{
        const itemlist: Monster[] =initItemlist(); 
        const item =itemlist.filter(item => item.place == place&&item.limit<=limit&&item.type==type);
       return item;
    }
    export function getItem(id:number) :Monster{
      const itemlist: Monster[] =initItemlist(); 
