...
function init(methodExecutor:MethodExecutor): void {
...
async function complete(session: any,ctx: Context): Promise<string> {
  ...
  async function commandList(session: any,ctx: Context): Promise<string> {
...
// /战斗配置
async function battleConfig(session: any,ctx: Context): Promise<string> {
  return '暂无';
}
...
async function info(session: any,ctx: Context): Promise<string> {
  const match = splitStringBySpace(extractContentAfterSecondSpace(session.content));
  if(match.length<2){
    return String(h('at',{id:(session.userId)}))+'请告诉我你要查看的物品（/查看 XXX）';
  }
